...
function EventDetail({ event, related, onClose }: { event: HqLiveEvent; related: HqLiveEvent[]; onClose: () => void }) {
  ...
  return <aside className="hq-live-detail" aria-label="Event detail">
    ...
    {tab === "overview" ? <div className="hq-live-detail__content">
      ...
      <div className="hq-live-detail__actions">{event.subject?.type === "member" && event.subject.id ? <Link className="hq-btn hq-btn--primary" to={`/hq/members/${encodeURIComponent(String(event.subject.id))}`}>View Member 360</Link> : null}<Link className="hq-btn hq-btn--ghost" to="/hq/audit">View in Audit Log</Link><button type="button" className="hq-icon-btn" aria-label="More event actions">•••</button></div>
...
function LiveEventsFeed({ allBrands, currentBrand, onAllBrandsChange }: { allBrands: boolean; currentBrand: string; onAllBrandsChange: (allBrands: boolean) => void }) {
  ...
  const todayStart = new Date(feedNow);
  todayStart.setHours(0, 0, 0, 0);
  const todayEvents = events.filter((event) => new Date(event.occurred_at).getTime() >= todayStart.getTime());
  ...
  return <>
    ...
    <section className="hq-live-kpis" aria-label="Live event summary"><EventKpi icon={Activity} tone="slate" value={(lastHour.length / 60).toFixed(1)} label="Events / min" detail="Derived from events loaded in the last 60 minutes; trend compares the previous 30 minutes." trend={allEventsTrend} active={metricFocus === "events"} onClick={() => focusMetric("events", "hour")} /><EventKpi icon={UserPlus} tone="green" value={String(registrations)} label="Registrations" detail="Registration events loaded today; trend compares the previous 30 minutes." trend={trendFor(registrationPredicate)} active={metricFocus === "registrations"} onClick={() => focusMetric("registrations", "today")} /><EventKpi icon={Heart} tone="rose" value={String(matches)} label="Matches" detail="Match events loaded today; trend compares the previous 30 minutes." trend={trendFor(matchPredicate)} active={metricFocus === "matches"} onClick={() => focusMetric("matches", "today")} /><EventKpi icon={MessageCircle} tone="blue" value={String(conversations)} label="Conversations" detail="Conversation events loaded today; trend compares the previous 30 minutes." trend={trendFor(conversationPredicate)} active={metricFocus === "conversations"} onClick={() => focusMetric("conversations", "today")} /><EventKpi icon={Shield} tone="green" value={String(reports)} label="Reports" detail="Report events loaded today; trend compares the previous 30 minutes." trend={trendFor(reportPredicate)} active={metricFocus === "reports"} onClick={() => focusMetric("reports", "today")} /><EventKpi icon={AlertTriangle} tone="red" value={String(errors)} label="Errors" detail="Critical or failed events loaded in the last hour; trend compares the previous 30 minutes." trend={trendFor(errorPredicate)} active={metricFocus === "errors"} onClick={() => focusMetric("errors", "hour")} /><EventKpi icon={Users} tone="teal" value="—" label="Active users" detail="Live presence is not currently instrumented." unavailable /></section>
