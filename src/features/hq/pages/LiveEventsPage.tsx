...
function EventKpi({ icon: Icon, tone, value, label, detail, trend, unavailable = false }: { icon: typeof Activity; tone: string; value: string; label: string; detail: string; trend?: number | null; unavailable?: boolean }) {
...
function LiveEventsFeed({ allBrands, currentBrand, onAllBrandsChange }: { allBrands: boolean; currentBrand: string; onAllBrandsChange: (allBrands: boolean) => void }) {
  ...
  const errorPredicate = (event: HqLiveEvent) => event.severity === "critical" || eventLooksLike(event, ["error", "failed"]);
  ...
  return <>
    ...
    <section className="hq-live-kpis" aria-label="Live event summary"><EventKpi icon={Activity} tone="slate" value={(lastHour.length / 60).toFixed(1)} label="Events / min" detail="Derived from events loaded in the last 60 minutes; trend compares the previous 30 minutes." trend={allEventsTrend} /><EventKpi icon={UserPlus} tone="green" value={String(registrations)} label="Registrations" detail="Registration events loaded today; trend compares the previous 30 minutes." trend={trendFor(registrationPredicate)} /><EventKpi icon={Heart} tone="rose" value={String(matches)} label="Matches" detail="Match events loaded today; trend compares the previous 30 minutes." trend={trendFor(matchPredicate)} /><EventKpi icon={MessageCircle} tone="blue" value={String(conversations)} label="Conversations" detail="Conversation events loaded today; trend compares the previous 30 minutes." trend={trendFor(conversationPredicate)} /><EventKpi icon={Shield} tone="green" value={String(reports)} label="Reports" detail="Report events loaded today; trend compares the previous 30 minutes." trend={trendFor(reportPredicate)} /><EventKpi icon={AlertTriangle} tone="red" value={String(errors)} label="Errors" detail="Critical or failed events loaded in the last hour; trend compares the previous 30 minutes." trend={trendFor(errorPredicate)} /><EventKpi icon={Users} tone="teal" value="—" label="Active users" detail="Live presence is not currently instrumented." unavailable /></section>
